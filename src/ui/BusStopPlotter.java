...
public class BusStopPlotter extends MapViewOverlay {
    ...
    public void markStops(Location currentLocation) {
        ...
        updateVisibleArea();
        newStopClusterer();
        Iterator stopManagerIter = StopManager.getInstance().iterator();
        while (stopManagerIter.hasNext()) {
            Stop stop = (Stop) stopManagerIter.next();
            LatLon stopLatLon = stop.getLocn();
            if (rectangleContainsPoint(northWest, southEast, stopLatLon)) {
                markStopsWhenInRange(stopIconDrawable, stop, stopLatLon);
            }
        }
    }

    /**
     * Mark all visible stops in stop manager onto map when they are in the range.
     */
    private void markStopsWhenInRange(Drawable stopIconDrawable, Stop stop, LatLon stopLatLon) {
        Marker marker = new Marker(mapView);
        marker.setPosition(gpFromLatLon(stopLatLon));
        marker.setIcon(stopIconDrawable);
        marker.setRelatedObject(stop);

        String stopName = stop.getName();
        String stopNumber = Integer.toString(stop.getNumber());
        marker.setTitle("Stop Number: " + stopNumber + "\nStop Name: " + stopName);
    ...
    public void updateMarkerOfNearest(Stop nearest) {
        ...
        // if already have a green labeled stop, set it to null
        // avoid several green labeled stop appears at the same time
        if (nearestStnMarker != null) {
            nearestStnMarker.setIcon(stopIconDrawable);
            nearestStnMarker = null;
        }

        if (stopMarkerMap.get(nearest) != null && nearest != null) {
